# -*- coding: utf-8 -*-
"""
Created on Fri Apr 17 00:03:00 2020
...
class ATM:
    ...
    def callTransferFunds(self):
        ...
        self.bankAccount.promptDonorAccount()
        self.screen.displayAccountsList(self.bankAccount.user_accounts_list)
        donor = input()
        transferAmount = 0
        ...
        for account in self.bankAccount.user_accounts_list:
            if account.getAccountName() == answer:
                ...
                while int(transferAmount) > account.getBalance():
                    self.screen.displayOverWithdrawal()
                    transferAmount = input()

        #Select recipient account.
        self.promptRecipientAccount()
        recipient = input()
        
        #Find recipient account, confirm, then transfer. 
        for account in self.bankAccount.user_accounts_list:
            if account.getAccountName() == answer:
                verify = self.confirmTransfer()
                #input validation & break in case "no" goes here.
                if verify = True:
                  donor.decreaseBalance(transferAmount)
                  recipient.increasebalance(transferAmount)
